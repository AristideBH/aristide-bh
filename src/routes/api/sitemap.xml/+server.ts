...
const filter = {
	_and: [{ status: { _nin: ['archived', 'draft'] } }, { seo: { _nnull: true } }]
};
...
export const GET: RequestHandler = async ({ fetch }) => {
	...
	try {
		...
		const homepage = await directus.request(getHomepage({ fields: ['seo', 'date_updated'] }));
		...
		const convertToSitemapEntries = (
			...
			return items
				...
				.map((item) => {
					...
					if ((item as Page).permalink !== undefined && (item as Page).permalink) {

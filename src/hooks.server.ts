...
async function refreshAccessToken(cookies: Cookies) {
	...
	cookies.set(
		'access_token',
		data.access_token,
		constructCookieOpts(Math.floor(data.expires / 1000))
	);
}

function isTokenExpired(jwtPayload: JwtPayload | null) {
	return jwtPayload?.exp
		? jwtPayload.exp < Math.floor(Date.now() / 1000) + TOKEN_EXPIRATION_BUFFER
		: true;
}
...
function buildCSP(url: URL) {
	...
	if (env.PUBLIC_UMAMI_SRC) {
		try {
			...
			console.error(
				"PUBLIC_UMAMI_URL n'est pas une URL valide. La CSP est peut-être incomplète.",
				e
			);
		}
	}
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	if (cookies.get('access_token') || cookies.get('refresh_token')) {
		let jwtPayload = cookies.get('access_token')
			? (jwt.decode(cookies.get('access_token') || '') as jwt.JwtPayload | null)
			: null;

		if (isTokenExpired(jwtPayload) || !cookies.get('access_token')) {
			try {
				await refreshAccessToken(cookies);
				jwtPayload = cookies.get('access_token')
					? (jwt.decode(cookies.get('access_token') || '') as jwt.JwtPayload | null)
					: null;
			} catch {
				cookies.delete('refresh_token', { path: '/' });
